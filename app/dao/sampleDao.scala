...
class sampleDao  @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends
  ...
  def getSampleByPosition(userId:Int,proId:Int,state:Int,input_type:String) : Future[Seq[SampleRow]] = {
    db.run(Sample.filter(_.accountid === userId).filter(_.projectid === proId).filter(_.state === state).
      filter(_.inputType === input_type).result)
  }
